export default {
  async fetch(request, env, ctx) {
    ...
    <style>
      ...
      /* === NUOVO CSS CONTEGGIO CARATTERI === */
      
      /* Contatore Barra Chat */
      #chat-char-floating {
        position: fixed;
        bottom: 55px; /* Appena sopra la barra input */
        left: 50%;
        ...
        font-weight: bold;
        z-index: 25001;
        pointer-events: none; /* Così ci puoi cliccare attraverso se serve */
        display: none; /* Nascosto di default */
        border: 1px solid #ffcc00;
      }

      /* Contatore dentro il Blocco Note Popup */
      #note-counter-bar {
        background: #333;
        color: #ffcc00;
        text-align: center;
      ...
      @media screen and (orientation: landscape) and (max-height: 500px) {
        ...
        /* Fix posizione contatore landscape */
        #chat-char-floating { bottom: 42px; font-size: 12px; padding: 2px 8px; }
      }
    </style>
    `;
    ...
    document.addEventListener('DOMContentLoaded', function() {
            ...
            if (btn) {
                ...
                newIcon.onclick = function() {
                    ...
                    if (content) {
                        ...
                        // === NUOVO CODICE: CONTEGGIO CARATTERI NEL BLOCCO NOTE (O SIMILI) ===
                        // Cerca una textarea dentro il contenuto appena spostato nel popup
                        const txtArea = popupInner.querySelector('textarea');
                        if(txtArea) {
                            // Crea la barra di conteggio
                            const counterBar = document.createElement('div');
                            counterBar.id = 'note-counter-bar';
                            counterBar.innerText = 'Caratteri: ' + txtArea.value.length;
                            
                            // Inseriscila PRIMA del contenuto (in cima al popup)
                            popupInner.insertBefore(counterBar, popupInner.firstChild);
                            
                            // Aggiorna quando scrivi
                            txtArea.addEventListener('input', function() {
                                counterBar.innerText = 'Caratteri: ' + this.value.length;
                            });
                        }
                        // ===================================================================
                    }
      ...
      // === NUOVO CODICE: CONTEGGIO CARATTERI CHAT PRINCIPALE ===
      const azioneInput = document.getElementById('azione');
      if (azioneInput) {
          // Crea il pallino fluttuante
          const floatCounter = document.createElement('div');
          floatCounter.id = 'chat-char-floating';
          document.body.appendChild(floatCounter);
          
          azioneInput.addEventListener('input', function() {
              const len = this.value.length;
              if (len > 0) {
                  floatCounter.style.display = 'block';
                  floatCounter.innerText = len;
              } else {
